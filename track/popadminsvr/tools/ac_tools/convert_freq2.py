...
def convert(input_file, output_file, camp_url):
    f = open(input_file, "r")
    js = f.read()
    f.close()
...
if len(sys.argv) < 2:
    print 'usage: %s campaign_url'%sys.argv[0]
    exit(-1)

input_file_name = '.pop_freq.js'
...
convert(input_file_name, output_file_name, cpurl)
